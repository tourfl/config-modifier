...
def config():
    ...
    my_conf = {
        "page1": {
            "initialSettings": {"coordinates": [10, 12], "color": "white"},
            "available-filters": {"name-filter": {"column": "name", "sort": "desc"}},
        }
    }
...
def config2():
    ...
    my_conf = {
        "page1": {
            "initialSettings": {"coordinates": [10, 12], "color": "green"},
            "available-filters": {"name-filter": {"column": "name", "sort": "asc"}},
            "data": [{"x": 10, "value": 100}, {"x": 20, "value": 200}],
        }
    }
...
def config3():
    ...
    my_conf = {
        "vessels": [
            {"coordinates": [13, 15], "name": "flying dutch"},
            {"coordinates": [42, 21], "name": "maltese hawk"},
        ]
    }
...
def test_change_value_OK(config):
    """
    change affects one config value
    """
...
def test_wrong_path_KO(config):
    """
    change affects an unexisting path -- expects no change
    """
    initial_config = copy.deepcopy(
        config
    )  # copy in order to be sure that no change occured
...
def test_change_array_OK(config):
    """
    change affects an array
    """
    change = {"page1.initialSettings.coordinates[0]": {"value": "Wow JSON injected!!"}}
    modify_config(config, change)

    assert config["page1"]["initialSettings"]["coordinates"][0] == {
...
def test_change_array_3_OK(config3):
    """
    change affects arrays at two different levels
    """
...
def test_index_out_of_range_KO(config):
    """
    change affects an array but it's out of range
    """
